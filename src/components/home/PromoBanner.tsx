...
const FALLBACK_BANNERS: PromoBanner[] = [
  {
    ...
    expiresAt: new Date(Date.now() + 1000 * 60 * 60 * 47 + 1000 * 60 * 33).toISOString(),
...
function TimerUnit({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="relative w-12 h-13 sm:w-14 sm:h-15 flex items-center justify-center rounded-xl bg-white/10 border border-white/15 overflow-hidden" style={{ height: "52px" }}>
...
function BannerSlide({ banner, active }: { banner: PromoBanner; active: boolean }) {
  const timeLeft = useCountdown(banner.timerEnabled ? banner.expiresAt : undefined)
  ...
  return (
    ...
    >
      ...
      <div className="container relative z-10 h-full flex items-center">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center w-full py-12 lg:py-16">
          ...
          <div className="space-y-5 text-center lg:text-left order-2 lg:order-1">
            ...
            <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-white leading-tight whitespace-pre-line">
              {banner.headline}
            </h2>
            ...
            {banner.discountBadge && (
              <div>
                <span
                  className="inline-block font-extrabold text-sm tracking-wide px-4 py-1.5 rounded-full text-white"
                  style={{ backgroundColor: accent }}
                >
                  {banner.discountBadge}
                </span>
            ...
            {banner.timerEnabled && (
              <div className="pt-1">
                {timeLeft.expired ? (
                  /* Expired state */
                  <div className="inline-flex items-center gap-2 text-sm font-semibold text-white/40 border border-white/10 rounded-full px-4 py-2">
            ...
            {banner.ctaLink && (
              <div className="pt-2">
                <Link
                  href={banner.ctaLink}
                  className="inline-flex items-center gap-2.5 font-bold text-sm px-7 py-3.5 rounded-full text-white transition-all duration-200 hover:scale-105 hover:shadow-xl active:scale-100"
                  style={{ backgroundColor: accent }}
                >
          ...
          <div className="relative flex items-center justify-center order-1 lg:order-2">
            <div className="relative w-52 h-52 sm:w-72 sm:h-72 lg:w-full lg:h-[360px]">
              {imageUrl ? (
                <Image
                  src={imageUrl}
                ...
                <div className="w-full h-full flex items-center justify-center opacity-10">
                  <svg viewBox="0 0 200 200" className="w-48 h-48 text-white fill-current">
...
export function PromoBanner({ banners: bannersProp }: PromoBannerProps) {
  ...
  useEffect(() => { setBarKey((k) => k + 1) }, [current])
  ...
  return (
    <section
      ...
      style={{ minHeight: "min(58vh, 540px)" }}
    ...
    >
      ...
      {banners.map((banner, i) => (
        <BannerSlide key={banner._id} banner={banner} active={i === current} />
      ))}
      ...
      {banners.length > 1 && (
        <>
          ...
          <button
            onClick={prev}
            className="absolute left-3 sm:left-5 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center transition-colors"
          ...
          <button
            onClick={next}
            className="absolute right-3 sm:right-5 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center transition-colors"
